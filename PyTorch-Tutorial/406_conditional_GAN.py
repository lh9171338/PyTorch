...
for step in range(10000):
    ...
    if step % 200 == 0:  # plotting
        ...
        plt.text(-.5, 2.3, 'D accuracy=%.4f (0.5 for D to converge)' % prob_artist0.data.numpy().mean(), fontdict={'size': 13})
        plt.text(-.5, 2, 'D score= %.4f (-1.38 for G to converge)' % -D_loss.data.numpy(), fontdict={'size': 13})
